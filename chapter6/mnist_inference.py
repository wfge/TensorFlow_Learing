...
def inference(input_tensor,train,regularizer):
    with tf.variable_scope('layer1-conv1'):
        ...
        conv1_biases=tf.get_variable("bias",[CONV1_DEEP],initializer=tf.constant_initializer(0.0))    #这可以看出卷积核的深度每个核的权重都是随机生成的，所以在每个深度上是不一样的，但是同一深度上由于参数共享，应该还是一样的
    ...
    with tf.variable_scope('layer5_fc1'):
        ...
        if train:fc1=tf.nn.dropout(fc1,0.5)            #在训练时才dropout,在测试时，每一个神经网络单元都是激活的
